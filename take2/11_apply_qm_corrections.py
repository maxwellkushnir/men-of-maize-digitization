"""
Men of Maize — Auto-correct Q=M≠C cases
Finds every position where Qwen and Mistral agree but Claude differs,
and applies the Q/M reading to men_of_maize_clean.txt and
men_of_maize_structured.json.

Rule: Qwen and Mistral are independent sources. Where they agree and Claude
differs, trust Q/M and correct Claude — no manual photo check needed.

Conservatism rules (correction is skipped if any fail):
  - Q/M phrase must be >= 2 words
  - The Claude phrase must appear EXACTLY ONCE in the relevant page text
  - The replacement must actually differ from the current text

Outputs:
  output/qm_corrections_log.txt  — full log of applied and skipped corrections
  (also updates men_of_maize_clean.txt and men_of_maize_structured.json in place)

Usage:
    python3 11_apply_qm_corrections.py [--dry-run]
"""
...
LOG_FILE    = OUTPUT_DIR / "qm_corrections_log.txt"

ALL_PDFS = ["2-25.pdf", "26-55.pdf", "56-75.pdf", "76-107.pdf", "108-.pdf"]

MIN_PHRASE_WORDS = 2   # skip single-word corrections (too risky without context)
...
def get_c_span(q_i1, q_i2, q_to_c, c_words):
    """Return (c_start, c_end) indices into c_words for the Q span [q_i1:q_i2]."""
    c_positions = [q_to_c[qi] for qi in range(q_i1, q_i2) if qi in q_to_c]
    if not c_positions:
        # Try anchors just outside the span
        c_before = None
        for qi in range(q_i1 - 1, max(-1, q_i1 - 10), -1):
            if qi in q_to_c:
                ...
                break
        c_after = None
        for qi in range(q_i2, min(q_i2 + 10, q_i2 + len(q_to_c))):
            if qi in q_to_c:
                c_after = q_to_c[qi]
                break
...
def find_qmc_cases(qwen_spreads, mistral_spreads, claude_pages):
    cases = []
    keys = sorted(
    ...
    for key in keys:
        ...
        for tag, i1, i2, j1, j2 in sm_qm.get_opcodes():
            ...
            q_phrase_norm = " ".join(norm_words(q_words[i1:i2]))
            c_phrase_norm = " ".join(norm_words(c_words[c_start:c_end]))

            if q_phrase_norm == c_phrase_norm:
                continue  # Claude already matches — no correction needed

            cases.append({
                "pdf":      pdf_name,
                "spread":   spread_num,
                "pages":    sorted(set(pages)),
                "q_phrase": " ".join(q_words[i1:i2]),   # display form
                "c_phrase": " ".join(c_words[c_start:c_end]),
                "q_norm":   q_phrase_norm,
                "c_norm":   c_phrase_norm,
            })
...
def apply_to_clean_txt(cases, path: Path):
    ...
    # Build page-boundary index for targeted replacement
    page_matches = list(re.finditer(r'^\[Page\s+(\d+)[^\]]*\]', text, re.MULTILINE))
    page_positions = {int(m.group(1)): (m.start(), page_matches[i+1].start() if i+1 < len(page_matches) else len(text))
                      for i, m in enumerate(page_matches)}

    for case in cases:
        c_phrase = case["c_phrase"]
        q_phrase = case["q_phrase"]
        pages    = case["pages"]

        if not c_phrase.strip() or not q_phrase.strip():
            skipped += 1
            log_lines.append(f"SKIP (empty phrase)  spread {case['spread']} of {case['pdf']}")
            continue

        # Restrict search to the relevant page range in the text
        ...
        if not page_starts:
            skipped += 1
            log_lines.append(f"SKIP (pages not in text)  {case['pdf']} spread {case['spread']}  C={repr(c_phrase)}")
            continue
        ...
        count = region.count(c_phrase)
        if count != 1:
            skipped += 1
            log_lines.append(f"SKIP (found {count}×)  {case['pdf']} spread {case['spread']}  C={repr(c_phrase[:60])}")
            continue

        new_region = region.replace(c_phrase, q_phrase, 1)
        text = text[:region_start] + new_region + text[region_end:]
        applied += 1
        log_lines.append(f"APPLY  {case['pdf']} spread {case['spread']}  p{pages}  {repr(c_phrase[:50])} → {repr(q_phrase[:50])}")
...
def apply_to_json(cases, path: Path):
    ...
    # Build page_num → page_index mapping
    ...
    for case in cases:
        ...
        # Find all content blocks across relevant pages
        target_blocks = []
        for pn in pages:
            ...
            for block in page_obj.get("content_blocks", []):
                if c_phrase in block.get("text", ""):
                    target_blocks.append(block)

        if len(target_blocks) != 1:
            skipped += 1
            log_lines.append(f"SKIP JSON (found in {len(target_blocks)} blocks)  {case['pdf']} spread {case['spread']}  C={repr(c_phrase[:50])}")
            continue

        target_blocks[0]["text"] = target_blocks[0]["text"].replace(c_phrase, q_phrase, 1)
        applied += 1
        log_lines.append(f"APPLY JSON  {case['pdf']} spread {case['spread']}  {repr(c_phrase[:50])} → {repr(q_phrase[:50])}")
...
def main():
    ...
    print("Finding Q=M≠C cases …")
    cases = find_qmc_cases(qwen_spreads, mistral_spreads, claude_pages)
    print(f"  {len(cases):,} cases found")

    print("Applying corrections to men_of_maize_clean.txt …")
    txt_applied, txt_skipped, txt_log = apply_to_clean_txt(cases, CLAUDE_FILE)
    ...
    print(f"These Q=M≠C cases no longer require manual photo review.")
    print(f"Remaining manual review queue: ~4,245 Q≠M spans in divergence_review.html")
